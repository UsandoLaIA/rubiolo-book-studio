...
const products: Record<ProductKey, { code: ProductKey; short: string; name: string; dimensions: string; use: string; capacity: string; hero: string; images: string[] }> = {
  HO1028: { code: "HO1028", short: "Operación agrícola", name: "Tráiler carrozado para drones fumigadores", dimensions: "5 × 2,40 m", use: "Logística móvil para drones agrícolas", capacity: "Doble nivel operativo", hero: assetPath("/products/HO1028.jpg"), images: ["HO1028.jpg", "HO1028-1.jpg", "HO1028-2.jpg", "HO1028-4.jpg", "HO1028-5.jpg"] },
  HO1012: { code: "HO1012", short: "Carga pesada", name: "Semirremolque basculante homologado", dimensions: "9 × 2,60 m", use: "Auxilio mecánico y maquinaria pesada", capacity: "8 / 10 toneladas", hero: assetPath("/products/HO1012.jpg"), images: ["HO1012.jpg", "HO1012-2.jpg", "HO1012-4.jpg", "HO1012-6.jpg", "HO1012-8.jpg", "HO1012-10.jpg"] },
  HO1003: { code: "HO1003", short: "Transporte vial", name: "Semirremolque balancín homologado", dimensions: "9 × 2,60 m", use: "Transporte de vehículos y auxilio", capacity: "8 / 10 toneladas", hero: assetPath("/products/HO1003.jpg"), images: ["HO1003.jpg", "HO1003-1.jpg", "HO1003-3.jpg", "HO1003-5.jpg", "HO1003-8.jpg", "HO1003-12.jpg"] },
};
...
export default function Home() {
  ...
  function shotImage(shot: Shot) {
    ...
    return assetPath(`/products/${product.images[index]}`);
  }
        ...
        <div className="pdf-document">
          {selectedShots.map((shot, index) => {
            ...
            return <article key={shot.id} className={`pdf-page ${index === 0 ? "pdf-cover" : ""}`}>
              ...
              <div className="pdf-brand">{index > 0 && <img src={assetPath("/products/logo4.png")} alt="Acoplados Rubiolo"/>}<span>{product.code}</span></div>
    ...
    return (
      <main className="editor-shell">
        <header className="editor-header">
          <button className="brand-button" onClick={() => setMode("home")}><img src={assetPath("/products/logo4.png")} alt="Acoplados Rubiolo"/></button>
        ...
        <div className="editor-workspace">
          ...
          {shot && draft && <section className="editor-canvas-wrap">
            <div className={`editor-page ${activePage === 0 ? "editor-cover" : ""}`}>
              ...
              <div className="editor-page-copy"><div className="editor-page-brand">{activePage > 0 && <img src={assetPath("/products/logo4.png")} alt=""/>}<span>{product.code}</span></div><span className="page-kicker">{shot.category} · 0{activePage + 1}</span><h1>{draft.title}</h1><p>{draft.body}</p><dl><div><dt>Dimensiones</dt><dd>{product.dimensions}</dd></div><div><dt>Capacidad</dt><dd>{product.capacity}</dd></div><div><dt>Aplicación</dt><dd>{primaryContext}</dd></div></dl></div>
  ...
  if (mode === "wizard") {
    return (
      <main className="app-shell v2-shell">
        <header className="app-header"><button className="brand-button" onClick={() => setMode("home")}><img src={assetPath("/products/logo4.png")} alt="Acoplados Rubiolo"/></button><div className="project-name"><span>PROYECTO</span><strong>Book comercial · {product.code}</strong></div><div className="wallet-mini"><span>SALDO</span><b>{wallet.toLocaleString("es-AR")} créditos</b></div><div className="draft-status"><i /> BORRADOR GUARDADO</div><button className="close-button" onClick={() => setMode("home")}>×</button></header>
